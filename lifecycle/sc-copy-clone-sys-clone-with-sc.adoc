...
+
Wird das System nicht mehr benötigt, erfolgt die Bereinigung in folgenden Schritten:

+
.. Den SnapCenter Clone-Delete-Workflow für das SAP HANA Datenvolume einschließlich Datenbank-Shutdown ausführen.
+
[NOTE]
====
Der SnapCenter Clone-Lösch-Workflow löscht den  `/etc/fstab` Eintrag standardmäßig nicht.

Damit der  `/etc/fstab`Eintrag beim Löschvorgang des Klons automatisch entfernt wird, muss der Klon zunächst wiederhergestellt und als automatisch erkannte Ressource eingerichtet werden. Wenn dieser Schritt nicht durchgeführt wird, wird der  `/etc/fstab`Eintrag nicht automatisch gelöscht und muss manuell entfernt werden.

====
.. Stoppen Sie SAP HANA Services.
.. Den SnapCenter Clone-Delete-Workflow für das gemeinsam genutzte SAP HANA Volume ausführen.
