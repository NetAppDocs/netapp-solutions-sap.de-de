...
[[off-site-backup-andor-disaster-recovery-data-replication]]
...
Obwohl Reparatursysteme und Testsysteme für die Notfallwiederherstellung typischerweise mithilfe von SAP-Systemklonen bereitgestellt werden (die keine umfangreichen Nachbearbeitungsschritte erfordern), müssen diese Nachbearbeitungsschritte für aktualisierte Test- und Trainingssysteme angewendet werden, um die Koexistenz mit dem Quellsystem zu ermöglichen. Daher konzentriert sich dieses Dokument auf SAP-Systemaktualisierungsszenarien. Weitere Details zu den verschiedenen Anwendungsfällen finden Sie im technischen Bericht link:sc-copy-clone-introduction.html["Automatisierung von SAP HANA Systemkopie und Klonvorgängen mit SnapCenter"^].
