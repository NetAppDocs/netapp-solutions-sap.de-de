...
Alle nachfolgenden Schritte sind identisch mit dem Workflow zum Klonen aus dem primären Speicher, wie im Abschnitt „ beschrieben from primary storage with tenant name equal to SID.“
...
Die erforderlichen Schritte in SnapCenter sind identisch mit dem, was im Abschnitt “ beschrieben wurde from primary storage with tenant name equal to SID.“] Der Unterschied liegt im Recovery-Vorgang des Mandanten innerhalb des Skripts `sc-system-refresh.sh`.
