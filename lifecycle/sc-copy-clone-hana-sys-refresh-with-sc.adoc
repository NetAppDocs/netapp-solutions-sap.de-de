...
|  | SAP HANA-Verschlüsselung mit Persistenz | SFSS: Zunächst, oder wenn die Root-Schlüssel im Quellsystem geändert wurden, müssen die Root-Schlüssel-Backups im Zielsystem importiert werden, bevor die Wiederherstellung durchgeführt werden kann. LSS: Zunächst, oder wenn das Passwort für das Root-Schlüssel-Backup geändert wurde, muss das Passwort in der LSS-Konfiguration festgelegt werden. 
...
|  | SAP HANA-Verschlüsselung mit Persistenz | SFSS: Wenn eine DBID des Quellsystems im Zielsystem noch nicht existiert, muss die Systemdatenbank zuerst wiederhergestellt werden, bevor die Sicherung des Stammschlüssels dieses Mandanten importiert werden kann. LSS: Die LSS-Topologie wird während der Wiederherstellung der Mandantendatenbank(en) automatisch mit der HANA-Topologie synchronisiert. 
...
Alle SAP-HANA-Systeme müssen gemäß dem Konfigurationsleitfaden konfiguriert werden. link:../bp/hana-aff-nfs-introduction.html["SAP HANA auf NetApp AFF Systemen mit NFS"]Die SnapCenter und die SAP HANA-Ressourcen wurden gemäß der Best-Practice-Anleitung konfiguriert. link:../backup/hana-sc-generic-scope.html["Technischer Bericht: SAP HANA Backup and Recovery with SnapCenter"]Die
...
. Konfiguration des SAP HANA-Systems in SnapCenter wie beschrieben in link:../backup/hana-sc-generic-scope.html["Technischer Bericht: SAP HANA Backup and Recovery with SnapCenter"]
+
...
In einer Fibre Channel SAN-Konfiguration müssen Sie im nächsten Bildschirm das verwendete Protokoll angeben.
