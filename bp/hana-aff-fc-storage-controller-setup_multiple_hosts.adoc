...
NOTE: NetApp ASA-Systeme unterstützen NFS nicht als Protokoll. NetApp empfiehlt, ein zusätzliches AFF- oder FAS-System für die  `/hana/shared` und  `/lss/shared` Dateisysteme zu verwenden.
...
NOTE: Wenn der SAP Local Secure Store Service (LSS) verwendet wird, muss das  `/lss/shared/SID` Verzeichnis auf allen Hosts zugänglich sein. NetApp empfiehlt, ein Volume für das  `/lss/shared/SID` Verzeichnis zu erstellen und es per NFS auf allen Hosts einzubinden.
...
NOTE: Es ist nicht notwendig, LVM zu verwenden, um mehrere LUNs zu kombinieren, um die SAP HANA KPIs zu erfüllen, aber es wird empfohlen
...
vol create -volume FC5_shared -aggregate aggr1_1 -size 512g -state online -policy default -snapshot-policy none -junction-path /FC5_shared -encrypt false   -space-guarantee none -anti-ransomware-state disabled
....
