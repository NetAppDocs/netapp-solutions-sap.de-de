...
NOTE: Wenn der SAP Local Secure Store Service (LSS) verwendet wird, befindet sich das  `/lss/shared/SID`Verzeichnis, das Standardinstallationsverzeichnis von LSS, auf der lokalen Festplatte. In einer Disaster-Recovery-Umgebung mit festplattenbasierter Replikation empfiehlt NetApp, innerhalb des  `SID1_shared`Volumes eine zusätzliche LUN für das  `/lss/shared/SID`Verzeichnis zu erstellen, sodass sich alle Dateisysteme auf dem zentralen Speicher befinden.
...
NOTE: Es ist nicht notwendig, LVM mit mehreren LUNs zu verwenden, um die SAP HANA-KPIs zu erfüllen, es wird jedoch empfohlen.
...
| Deaktivieren Sie automatische Snapshot Kopien | vol modify -vserver <vserver-name> -volume <volname> -snapshot-policy none 
...
lun create -path /vol/FC5_shared/FC5_shared -size 260g -ostype linux -space-reserve disabled
....
...
lun map -path /vol/FC5_shared/FC5_shared -igroup HANA-FC5
....
