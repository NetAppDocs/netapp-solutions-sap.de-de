...
NOTE: SAP HANA-Systeme mit mehreren Hosts benötigen das  `/hana/shared` Dateisystem und die  `/lss/shared` Dateisysteme, falls der SAP Local Secure Store Service (LSS) verwendet wird, die mit allen Hosts eines Systems verbunden sind. Üblicherweise handelt es sich dabei um eine NFS-Freigabe, die von einem NFS-Server bereitgestellt wird. Es wird empfohlen, einen hochverfügbaren NFS-Server, z. B. ein NetApp FAS oder AFF System, zu verwenden. Eine weitere Option ist die Verwendung des integrierten NFS-Servers eines LINUX-Hosts dafür.
