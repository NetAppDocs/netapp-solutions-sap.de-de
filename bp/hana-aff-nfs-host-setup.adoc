...
NOTE: Die folgenden Beispiele zeigen eine SAP HANA-Datenbank mit SID=NF2 unter Verwendung von LSS.
...
Das nächste Beispiel zeigt eine SAP HANA-Datenbank mit SID=NF2, die NFSv4.1 für Daten- und Protokolldateisysteme und NFSv3 für die  `/hana/shared`, `/lss/shared`, und  `/usr/sap/NF2` Dateisysteme verwendet. Eine NFS-Übertragungsgröße von 1 MB für Lesevorgänge und 256k für Schreibvorgänge wird verwendet.
