...
Die von ONTAP Autonomous Ransomware Protection erstellten Snapshots können nicht für die Forward Recovery der SAP HANA Datenbank verwendet werden. Daher wird empfohlen, sie nicht für SAP HANA Daten- und Log-Volumes zu verwenden und die Funktion für diese Volumes zu deaktivieren. Verwenden Sie stattdessen den auf SnapCenter basierenden Datenschutz mit manipulationssicheren Snapshots zum Schutz vor Ransomware: link:../backup/hana-sc-generic-scope.html["Technischer Bericht: SAP HANA Backup and Recovery with SnapCenter"]. ONTAP Autonomous Ransomware Protection kann für andere Volumes wie /hana/shared von Vorteil sein.

Es ist möglich, die automatische Aktivierung dieser Funktion während der Volume-Erstellung für einen ONTAP Cluster zu deaktivieren, jedoch wird empfohlen, die Standardeinstellung beizubehalten und sie für Volumes zu deaktivieren, bei denen diese Funktionalität nicht benötigt wird.
...
NOTE: Wenn der SAP Local Secure Store Service (LSS) verwendet wird, befindet sich das `/lss/shared/SID` Verzeichnis, das Standardinstallationsverzeichnis von LSS, auf der lokalen Festplatte. In einer Disaster-Recovery-Umgebung mit festplattenbasierter Replikation empfiehlt NetApp, ein Verzeichnis innerhalb des `SID_shared` Volumes für das `/lss/shared/SID` Verzeichnis anzulegen, sodass sich alle Dateisysteme auf dem zentralen Speicher befinden.
...
| SID-shared | usr-sap + shared + lss-shared | /usr/sap/SID + /hana/shared + /lss/shared/SID 
|===
...
NOTE: Wenn der SAP Local Secure Store Service (LSS) verwendet wird, muss das  `/lss/shared/SID` Verzeichnis auf allen Hosts zugänglich sein. NetApp empfiehlt, ein Verzeichnis innerhalb des  `SID_shared` Volumes für das  `/lss/shared/SID` Verzeichnis zu erstellen.
...
| SID_freigegeben | lss-shared | /lss/shared/SID | Auf allen Hosts montiert 
