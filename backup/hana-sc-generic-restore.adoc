...
NOTE: Mit Azure NetApp Files werden Wiederherstellungsvorgänge für primäre ANF-Volumes oder ANF-Backups unterstützt. Für primäre ANF-Volumes wird ein Volume-Revert, für ANF-Backups eine Wiederherstellung vor Ort mit Wiederherstellung einzelner Dateien durchgeführt. In beiden Fällen werden die Konfigurationen der Anwendungsvolumegruppen beibehalten.


NOTE: Wenn die Datenträgerverschlüsselung aktiviert ist und der SAP Local Secure Store (LSS) verwendet wird, wird eine Wiederherstellung mit SnapCenter unterstützt, solange das Backup-Passwort des Stammschlüssels im LSS seit der Sicherung nicht geändert wurde. Wenn das Passwort geändert wurde und eine Wiederherstellung mit einem älteren Snapshot mit einem anderen Passwort durchgeführt wird, muss die Wiederherstellung manuell erfolgen und das alte Passwort muss in der Wiederherstellungsanweisung angegeben werden: „RECOVER DATA USING SNAPSHOT CLEAR LOG ENCRYPTION ROOT KEYS BACKUP PASSWORD 'old-password'“
