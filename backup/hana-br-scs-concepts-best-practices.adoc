...
Weitere Informationen zur Anpassung des Zeitplans des Bereinigungsjobs oder zum Auslösen einer manuellen Aktualisierung finden Sie im Abschnitt link:hana-br-scs-advanced-config-tuning.html#change-schedule["„Change Scheduling Frequency of Backup Synchronization with off-Site Backup Storage“."]
...
Das Backup-Aufbewahrungsmanagement für Protokolle ist standardmäßig aktiviert. Bei Bedarf kann er wie im Abschnitt beschrieben deaktiviert werden link:hana-br-scs-advanced-config-tuning.html#disable-auto["„Automatische Erkennung auf dem HANA-Plug-in-Host deaktivieren.“"]
